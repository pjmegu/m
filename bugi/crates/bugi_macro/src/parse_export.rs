...
use crate::{DataType, ParseResult};

fn parse_sd(i: &str) -> IResult<&str, DataType> {
    let (i, data) = ws(parse_string)(i)?;
...
pub fn parse_export(i: &str) -> IResult<&str, ParseResult> {
    let (i, res) = ws(separated_list0(ws(char(',')), parse_export_attr))(i)?;
